...
* `schedule_expression` - (Required, if `event_pattern` isn't specified) The scheduling expression. Must be less than or equal to 256 characters in length.
...
* `description` - (Optional) The description of the rule. Must be less than or equal to 512 characters in length.
